...
computeCellSize model =
    let
        ...
        maxSize =
            h - nonBoardSize model - (min maxKeypadSize <| minKeypadSize h)

        total =
            min (windowSize.width - 10) maxSize

        size =
            (total - 2 * (cellBorder + whiteSpace)) // rows
    in
    size
...
computeBoardSizes model =
    let
        ...
        keypadSize =
            h - boardSize - nonBoardSize model
    in
    { boardSize = boardSize
    , cellSize = cellSize
