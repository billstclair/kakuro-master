...
module PlayHelpers exposing (isAllDone, computeFilledCellClasses, possibilities)

import SharedTypes exposing (GameState, IntBoard, BClassMatrix, BClassBoard)
    ...
    if guess == 0 then
        ...
        in
            case idx of
                Nothing -> res
                Just i ->
                    let ( row_, col_ ) = incrementor (i + 1) ( row, col )
                    in
